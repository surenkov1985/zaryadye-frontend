document.addEventListener("DOMContentLoaded", function () {
	...
	if (!sessionStorage.hasOwnProperty("isLoad")) {
		showPreloader()
	} else {
	...
	async function showPreloader() {
		...
		let preloader = $("#preload"); // селектор прелоадера
		$(preloader).show()
		...
		if (imagesCount >= imgSum && imagesCount > 0) {
			...
			await $(".preload__counter")
				.circularProgress({
					percent: 0,
					starting_position: 0,
				})
				.circularProgress("animate", percent, 200);
			...
			async function img_load() {
				...
				loadedImg++;
				if (Math.round(progress) == 100 && loadedImg == imagesCount) {
					await preloader.delay(100).fadeOut("slow");
					dBody.css("overflow", "");
				}
				await $(".preload__counter").circularProgress("animate", progress, 200);
				// window.scroll.start();
	...
	fsSliders();
	// hallsSliders();
	// const hallsSlider = hallsSliders();
	// hallsSlider?.sliderMain.init();
	// hallsSlider?.sliderSide.init();
	// newsSlider();
	calendarSliders();
	eventSlider();
	personSliders();
	...
	const audioPlayer = document.querySelector(".fs__player_view");

	if (audioPlayer) {
		const audio = new Audio(audioPlayer.dataset.audio);
		let isPlaying = audio.played.length;
		let isStarted = false;
		audio.volume = 0.5;
		...
		audio.addEventListener("loadeddata", (event) => {
			...
			$(".audio_duration").text(`${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`);

			// $("#player_duration").slider("option", { 'max': duration });

			$("#player_duration").slider({
				animate: "slow",
				range: "min",
				max: duration,
				value: 0,
				slide: function (event, ui) {
					...
					const duration = Math.floor(ui.value);
					const minutes = Math.floor(duration / 60);
					const seconds = duration % 60;
					$(".audio_curr_time").text(`${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`);
		...
		$(document).on("click", ".sound_control .mute", function () {
			if ($(this).hasClass("muted")) {
				$("#player_sound").slider("value", audio.volume);
				$("#player_sound").slider("option", { disabled: false });
			} else {
				$("#player_sound").slider("value", 0);
				$("#player_sound").slider("option", { disabled: true });
			}
			$(this).toggleClass("muted");
			audio.muted = !!$(this).hasClass("muted");
		});
		...
		audio.addEventListener("timeupdate", (event) => {
			...
			const seconds = duration % 60;
			$(".audio_curr_time").text(`${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`);
			$("#player_duration").slider("value", Math.floor(audio.currentTime));
		});

		audio.addEventListener("ended", (event) => {
			$(".fs__player_vinil").removeClass("play").removeClass("paused");
			$("button.play .pause_icon").hide();
			$("button.play .play_icon").show();
			isPlaying = false;
		});
		audio.addEventListener("pause", (event) => {
			$(".fs__player_vinil").addClass("paused");
			$("button.play .pause_icon").hide();
			$("button.play .play_icon").show();
			isPlaying = false;
		});
		...
		audio.addEventListener("play", (event) => {
			isPlaying = true;
			if (!isStarted) {
				$(".fs__player_vinil").addClass("play");
				isStarted = true;
			}
			$(".fs__player_vinil").removeClass("paused");
			$("button.play .pause_icon").show();
			$("button.play .play_icon").hide();
		});
	...
	$(document).on("click", ".leaders .about__more_link", function (e) {
		...
		if (clampedText.offsetHeight < clampedText.scrollHeight) {
			// $(clampedText).css({ display: "inline" });
			// //console.log($(clampedText).innerHeight(true), clampedText.offsetHeight);
			gsap.to(clampedText, { maxHeight: clampedText.scrollHeight, duration: 0.3, ease: "none" }).then(() => {
				$(this).text("Свернуть");
			});
		} else {
			gsap.to(clampedText, { maxHeight: lineHeight * 10, duration: 0.3, ease: "none" }).then(() => {
				$(this).text("Читать всё");
			});
		}
	});
	const videoLogo = document.querySelectorAll(".video_logo");

	for (let logo of videoLogo) {
		const video = logo.querySelector("video");
		// video.play()
	}
	// new AjaxLoadHandler()
	// const historySlider = historySliders();
	// const hallsSlider = hallsSliders();
	locationSliders();
	const personal = personalSlider();
	const elmOverlay = document.querySelector(".shape-overlays");
	var overlay = new ShapeOverlays(elmOverlay);
	const themeOverlay = document.querySelector(".shape-overlays-theme");
	...
	(function () {
		window.scroll = new Lenis({
			...
			getDirection: true,
			prevent: (node) => {
				// console.log(!node.closest(".mobile") && $(node).css("overflow") === "auto" && $(node).css("overflow") === "scroll");
		...
		scroll.on("scroll", (args) => {
			...
			// if (Object.keys(args.currentElements).length) {
			// 	Object.keys(args.currentElements).forEach((item) => {
			// 		if ($(args.currentElements[item].target).hasClass("events__item_preview")) {
			// 			if (args.currentElements[item].progress >= minProgress) {
			// 				$(args.currentElements[item].target).addClass("isView");
			// 			}
			// 		}
			// 	});
			// }
	...
	if (localStorage.getItem("startElem") && document.getElementById(localStorage.getItem("startElem"))) {
		...
		// 	// offset: +localStorage.getItem('scrollPosY')
		// })
	...
	$(document).on("click", ".events__item a", function (e) {
		...
		localStorage.setItem("startElem", target);
	});
	...
	// $(document).on("click", ".events__nav_item", function (e) {
	// 	e.preventDefault();

	// 	const url = this.href.replace("https://new.zaryadyehall.ru", "");
	// 	const event = $("#event");
	// 	const cancelled = $("");
	// 	$.ajax({
	// 		method: "GET",
	...
	// 			const page = $(res).find("#event");
	// 			const newCancelled = $(res).find("#cancelled");
	// 			const faq = $(res).find(".faq");
	// 			//console.log(newCancelled);
	// 			$(event).replaceWith($(page));
	// 			if (newCancelled.length) {
	// 				$(cancelled).replaceWith($(newCancelled));
	// 			}
	// 			if (faq.length) {
	// 				cafeSliders();
	// 			}
		...
		$(".drag")
			.on("mousedown touchstart", function (e) {
				...
				if (e.touches.length > 1) {
					origin = [e.touches[0].x + (e.touches[0].x - e.touches[1].x) / 2, e.touches[0].y + (e.touches[0].y - e.touches[1].y) / 2];
				}
			})
			...
			.on("mousemove touchmove", function (e) {
				e.preventDefault();
				if (e.touches) {
					let dragHeight = $(this).height();
					let dragWidth = $(this).width();
					let scale = $(this).css("scale") != "none" ? $(this).css("scale") : 1;
					if (e.touches.length > 1) {
						if (!!leftTouchX && !!rightTouchX) {
							window.scroll.stop();
							let curLeftTouchX = Math.min(e.touches[0].screenX, e.touches[1].screenX),
								curRightTouchX = Math.max(e.touches[0].screenX, e.touches[1].screenX),
								curLeftTouchY = Math.min(e.touches[0].screenY, e.touches[1].screenY),
								curRightTouchY = Math.max(e.touches[0].screenY, e.touches[1].screenY);
							if (
								(leftTouchX > curLeftTouchX && rightTouchX < curRightTouchX) ||
								(leftTouchY > curLeftTouchY && rightTouchY < curRightTouchY)
							) {
								dragHeight += 10;
								dragWidth = dragHeight * 1.8;
								scale = +scale + 0.1;

								$(this).css({ scale: scale, "transform-origin": `${origin[0]}px ${origin[1]}px` });
							} else if (
								(leftTouchX < curLeftTouchX && rightTouchX > curRightTouchX) ||
								(leftTouchY < leftTouchY && rightTouchY > curRightTouchY)
							) {
								dragHeight -= 10;
								dragWidth = dragHeight * 1.8;
								scale = scale - 0.1 < 1 ? 1 : scale - 0.1;
								$(this).css({ scale: scale, "transform-origin": `${origin.join(" ")}` });
							}
						}
						leftTouchX = Math.min(e.touches[0].screenX, e.touches[1].screenX);
						rightTouchX = Math.max(e.touches[0].screenX, e.touches[1].screenX);
						leftTouchY = Math.min(e.touches[0].screenY, e.touches[1].screenY);
						rightTouchY = Math.max(e.touches[0].screenY, e.touches[1].screenY);
					}
				}
				if (drag) {
					var left = parseInt($(this).css("left")) || 0,
						...
						newLeft =
							e.touches && e.touches.length === 1
								? left + (e.touches[0].clientX - ($(this).data().startX || e.touches[0].clientX))
								: left + (e.clientX - ($(this).data().startX || e.clientX)),
						newTop =
							e.touches && e.touches.length === 1
								? top + (e.touches[0].clientY - ($(this).data().startY || e.touches[0].clientY))
								: top + (e.clientY - ($(this).data().startY || e.clientY)),
	...
	resizeObserver.observe(document.querySelector("#smooth-wrapper"));
...
function resizeScrollPath(width) {
	...
	if (width > 768) {
		$(scrollPath).attr(
			"d",
			`M0 ${$(icon).height() / 1.3}C${width / 2 / 1.25} ${$(icon).height() / 1.3} ${width / 2 / 1.25} 0 ${width / 2} 0C${width / 1.7} 0 ${width / 1.7
			} ${$(icon).height() / 1.33} ${width} ${$(icon).height() / 1.33}L${width} ${$(icon).height()}L0 ${$(icon).height()}L0 ${$(icon).height() / 1.3
			}Z`
		);
	} else {
		$(scrollPath).attr(
			"d",
			`M0 ${$(icon).height() / 1.21}C${width / 2 / 1.82} ${$(icon).height() / 1.21} ${width / 2 / 1.44} 0 ${width / 2} 0C${width / 1.53} 0 ${width / 1.38
			} ${$(icon).height() / 1.25} ${width} ${$(icon).height() / 1.25}L${width} ${$(icon).height()}L0 ${$(icon).height()}L0 ${$(icon).height() / 1.21
			}Z`
		);
	}
}
