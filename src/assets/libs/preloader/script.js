(function () {
    ...
    var methods = {
        init: function (options) {
            ...
            //     percentage.textContent = global_settings.percentage;
            // }
            ...
            if (global_settings.text != "percent") {
                text.textContent = global_settings.text;
            }
            ...
            this.style.height = global_settings.height;
            this.style.width = global_settings.width;
            this.classList.add("circular-progress-bar");

            // Remove old canvas
            var oldCanvas = this.querySelector("canvas");
            if (oldCanvas) {
                this.removeChild(oldCanvas);
            }

            // Put canvas inside this
            this.appendChild(createCanvas(this));
        ...
        animate: function (value, time) {
            ...
            var theInterval = setInterval(function () {
                if (global_settings.percent < value) {
                    // Remove old canvas
                    var oldCanvas = scope.querySelector("canvas");
                    if (oldCanvas) {
                        scope.removeChild(oldCanvas);
                    }
                    // Increment percent
                    global_settings.percent += percent_change;
                    // Put canvas inside this
                    scope.appendChild(createCanvas(scope));
                } else {
    ...
    function circularProgress(methodOrOptions) {
    ...
    function removeUnit(apples) {
        if (apples.indexOf("px") > -1) {
            return apples.substring(0, apples.length - 2);
        } else if (apples.indexOf("%") > -1) {
    ...
    function insertText(scope) {
        var percentageElement = scope.querySelector(".progress-percentage");
        if (percentageElement && Math.round(global_settings.percent) <= 100) {
            percentageElement.textContent = Math.round(global_settings.percent) + "%";
        }
    }
    // create canvas
    ...
    // Attach the circularProgress function to the global scope
...
(function ($) {
	...
	// Put percentage or custom text inside progress circle
	function insertText(scope) {
		// console.log(global_settings.percent);
		if (Math.round(global_settings.percent) <= 100) {
