...
const elements = findElementsWithBackgroundImage();
jQuery("html").hide();
(function ($) {
		...
		ToggleImages: function () {
			...
			$(elements).each(function () {
				if ($special.images) {
					...
					console.log(111);
					if ($(this).data("src")) $(this).css({ "background-image": $(this).data("src") });
					
				} else {
					$(this).data("src", $(this).css("background-image"));
					...
					switch (parent) {
						...
						case "audio":
							if ($(this).val() == 1) {
								$("i.special-audio").remove();
								if (responsiveVoice.isPlaying()) responsiveVoice.cancel();
								$("p,h1,h2,h3,h4,h5,h6,li,dt,dd,.audiotext,a,b").off();
								$(this).val(0);
							} else {
	...
	var ResponsiveVoice = function () {
			...
			a.fallback_finishPart = function (b) {
				a.isFallbackAudioPlaying()
					...
					: (a.checkAndCancelTimeout(),
					  a.fallback_part_index < a.fallback_parts.length - 1 ? (a.fallback_part_index++, a.fallback_startPart()) : a.speech_onend());
			};
			a.cancel = function () {
				a.checkAndCancelTimeout();
				a.fallbackMode
